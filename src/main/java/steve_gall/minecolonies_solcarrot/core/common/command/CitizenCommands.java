...
public class CitizenCommands
{
	...
	public class EatensCommands
	{
		public static LiteralArgumentBuilder<CommandSourceStack> register()
		{
			var command = Commands.literal("eatens");
			command.then(show());
			command.then(clear());
		...
		{
			return literal("clear", false, (context, citizen) ->
			{
				var foodData = IMineColoniesSoLAPI.instance().getFoodData(citizen);
		...
		{
			return literal("show", false, (context, citizen) ->
			{
				var foodData = IMineColoniesSoLAPI.instance().getFoodData(citizen);
				var tags = ItemUtils.serializeItems(foodData.getEatens());
				context.getSource().sendSystemMessage(Component.literal("========== " + tags.size()));

				for (var i = 0; i < tags.size(); i++)
				{
					var tag = tags.get(i);
					context.getSource().sendSystemMessage(Component.literal(i + " - " + tag.getAsString()));
				}
